...
%  \draw  [-] (xe1) -- (fr);
  ...
  \path (ic) ++(0,-0.5cm) +(1cm,0) coordinate(xi1) +(-1cm,0) coordinate(xi2);
  \draw[-,name path=line1,bordeau,thick] (i1) |-node[above,xshift=-2.5cm]{$\alpha_1$} (xi1) -- (ic); %here
  \draw[-,name path=line1,blue,thick]  (ic) -- (xi2)node[above,xshift=-2.5cm]{$\alpha_2$} -| (i2); %here
  \path[-,name path=line1] (i1) |- (xi1) -- (ic) -- (xi2) -| (i2); %here
